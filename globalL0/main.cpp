...
int main() {
    ...
    point_cloud::ReadXYZPoints("../data/foam_box.xyz", &points);
    ...
    Array<RPoint3D> s_points;
    Array<RVector3D> s_normals;
    for (int i = 0; i < points.size(); i++)
    {
        if(labels[i] == -1) continue;
        s_points.push_back(points[i]);
        s_normals.push_back(planes[labels[i]].normal());
    }
    ...
    cl::point_cloud::WriteXYZPoints("../data/out.xyz", s_points, s_normals);
